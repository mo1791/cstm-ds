...
//  -----------------------------------------------------------------------
	/** PARAM CTOR **/
	node( T const& data ) noexcept requires( std::copy_constructible<T> )
		: m_data( data )
		, m_left( nullptr )
		, m_right( nullptr )
		, m_parent( nullptr )
...
//  -----------------------------------------------------------------------
	...
	node( T && data ) noexcept requires( std::move_constructible<T> )
...
//  -----------------------------------------------------------------------
	/** C++20 RANGE TYPE CTOR  **/
	template <rng::range R>
	binary_search_tree( R && /* range */ ) requires( std::convertible_to<rng::range_value_t<R> ,T> );
//  -----------------------------------------------------------------------
	/**  ITERATOR TYPE CTOR **/
	template <std::input_iterator I, std::sentinel_for<I> S>
	binary_search_tree( I /* begin */, S /* end */ ) requires( std::convertible_to<std::iter_value_t<I> ,T> );
//  -----------------------------------------------------------------------
	/** COPY CTOR  **/
...
//  -----------------------------------------------------------------------
	...
	template <class U>
	void insert( U && /* data */ ) requires( std::convertible_to<std::decay_t<U>, T> );
//  -----------------------------------------------------------------------
	/** INSERTING A NODE IN TREE ( construct in place ) **/
	template <class... Args>
	void emplace( Args && ... /* args */ ) requires ( is_class<T> && std::constructible_from<T, Args...> );
//  -----------------------------------------------------------------------
//  -----------------------------------------------------------------------
		...
		if ( p_root != nullptr )
		{
			try
			{
				...
				while ( p_root != nullptr )
				{
					...
					if ( ( p_root->m_right != nullptr ) && ( not v_copy->m_right ) )
					{
...
//  -----------------------------------------------------------------------
	    ...
	    : binary_search_tree()
	{
	    ...
	    rng::for_each( p_begin ,p_end, std::bind( &binary_search_tree::insert, this, _1 ) );
	}
//  -----------------------------------------------------------------------
//  -----------------------------------------------------------------------
...
//  -----------------------------------------------------------------------
	/** **/
	template <std::totally_ordered T> template <class ...Args>
